...
public class Sketch extends PApplet {
	...
	boolean cont=false;//anuncia cuando termina
	...
	public void draw(){
		...
		fill(225,0,225);
		String textp1 = Integer.toString(p1.getDisponibilidad());
		String textp2 = Integer.toString(p2.getDisponibilidad());
		String textp3 = Integer.toString(producto.getDisponibilidad());
		text(textp1,45,70);
		text(textp2,70,70);
		text(textp3,95,70);
		...
		if(boton_producir==true){
			...
			if(fabricando==false){
				cont=fabrica.fabricar();
				fabricando=true;}
			if(!cont){
				text("Disponibilidad agotada", 400, 400);
				return;
			}
