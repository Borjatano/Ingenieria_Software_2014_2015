...
public class Fabrica {
	...
	*/
	...
	public boolean fabricar(){
		...
		p1=solicitar(tipo1);
		if (p1==null)//si no encuentra pieza, termina
			return false;
		p2=solicitar(tipo2);
		if (p2==null)//si no encuentra pieza, termina
			return false;
		// CASO DE ERROR q no haya piezas  //////
		
		return producir(p1, p2, producto);
	...
	private Pieza solicitar(String tipo){
		...
		//try{
			...
			Pieza inicial, temp;
			inicial=almacen.damePieza(/*tipo*/);
			if (inicial==null)
				return null;
			temp=inicial;
			boolean cont=true;
			do{
				try{//Ve si la pieza es valida
					temp.prestate(tipo);
					cont=false;
				}catch(Exception e){//si no lo es, solicita otra
					temp=almacen.damePieza(/*tipo*/);
				}				
			}while (inicial!=temp && cont);//hasta que encuentra una valida o la inicial
			if(cont==false)//si se encuentra, se finaliza
				res=temp;
	...
	private boolean producir(Pieza p1, Pieza p2, String tipo_producto){
		...
		res=solicitar(tipo);
		if(res==null){
			//parent.text("no se encontro la pieza de tipo " + tipo, 400, 400);
			return false;
		}
